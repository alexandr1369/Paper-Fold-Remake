		...
		if (GUILayout.Button("Cut Object")) {
			...
			if (!recursiveSlice) {
				...
				if (hull != null) {
					...
					GameObject newHull;
					//Transform upperHullParentTransform, lowerHullParentTransform;

					/// UPPER HULL
					//upperHullParentTransform = new GameObject("Upper_Hull_Parent").transform;
					//upperHullParentTransform.rotation = (target as PlaneUsageExample).transform.rotation;
					newHull = hull.CreateUpperHull(source, crossMat);
					//newHull.transform.parent = upperHullParentTransform;

					/// LOWER HULL
					//lowerHullParentTransform = new GameObject("Lower_Hull_Parent").transform;
					//lowerHullParentTransform.rotation = (target as PlaneUsageExample).transform.rotation;
					newHull = hull.CreateLowerHull(source, crossMat);
					//newHull.transform.parent = lowerHullParentTransform;
